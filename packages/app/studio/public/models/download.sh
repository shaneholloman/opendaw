...
#
# This is a self-contained copy that writes into its own directory:
...
# Mirror: ModernMube/HTDemucs_onnx on Hugging Face
# ---------------------------------------------------------------------------
...
# Mirror: AEmotionStudio/basic-pitch-onnx-models on Hugging Face
# ---------------------------------------------------------------------------
...
for file in "${HTDEMUCS_DEST}" "${BASIC_PITCH_DEST}"; do
    if [[ -f "${file}" ]]; then
        printf "  %-60s %s\n" "${file#${MODELS_DIR}/}" "$(print_sha "${file}")"
    fi
done
